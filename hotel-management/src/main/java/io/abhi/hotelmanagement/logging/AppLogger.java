package io.abhi.hotelmanagement.logging;
...
public class AppLogger extends OncePerRequestFilter{
	...
	private static String logRequestBody(ContentCachingRequestWrapper request, String prefix) {
	...
	private static String logResponse(ContentCachingResponseWrapper response, ContentCachingRequestWrapper request,
			String prefix) {
	...
	/*
	 * @Before("within(io.abhi.hotelmanagement.controller)") public void
	 * endpointBefore(JoinPoint p) { if (log.isTraceEnabled()) {
	 * log.trace(p.getTarget().getClass().getSimpleName() + " " +
	 * p.getSignature().getName() + " START"); Object[] signatureArgs = p.getArgs();
	 * 
	 * 
	 * ObjectMapper mapper = new ObjectMapper();
	 * mapper.enable(SerializationFeature.INDENT_OUTPUT); try {
	 * 
	 * if (signatureArgs[0] != null) { log.trace("\nRequest object: \n" +
	 * mapper.writeValueAsString(signatureArgs[0])); } } catch
